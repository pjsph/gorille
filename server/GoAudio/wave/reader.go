...
func parseRawData(wfmt WaveFmt, rawdata []byte) []Frame {
    ...
    frames := make([]Frame, len(rawdata) / bytesSampleSize)
    // read the chunks
    ...
    for i := 0; i < len(frames); i += nbOfFramesPerRoutine {
        wg.Add(1)
        rawFramesPerRoutine := rawdata[i * bytesSampleSize : min(bytesSampleSize * (i + nbOfFramesPerRoutine), len(frames) * bytesSampleSize)]
...
func paral(startIndex int, frames []Frame, size int, wfmt WaveFmt, rawFrames []byte, wg *sync.WaitGroup){
    for i := 0; i < len(rawFrames); i += size {
        unscaledFrame := byteSizeToIntFunc[wfmt.BitsPerSample](rawFrames[i:i+size])
        scaled := scaleFrame(unscaledFrame, wfmt.BitsPerSample)
        frames[startIndex + i / size] = scaled
    }
    wg.Done()
}
